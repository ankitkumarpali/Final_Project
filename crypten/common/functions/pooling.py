...
def max_pool3d(
    ...
    """
    max_input = self.clone()
    max_input.data, output_size = _pool3d_reshape(
        self.data,
        kernel_size,
        padding=padding,
        stride=stride,
...
def _max_pool3d_backward(
    ...
    assert self.dim() == 5, "Input to _max_pool3d_backward must have 5 dimensions"
    assert (
        indices.dim() == 8
    ), "Indices input for _max_pool2d_backward must have 6 dimensions"
    ...
    # has non-zero value corresponding to the argmax of the corresponding
    # block of the max_pool2d input.
    kernels = self.view(self.size() + (1, 1, 1)) * indices
        ...
        for j in range(self.size(3)):
            for k in range(self.size(4)):
                ...
                result[
                    ...
                    left_ind : left_ind + k0 * d0 : d0,
                    top_ind : top_ind + k1 * d1 : d1,
                    dep_ind : dep_ind + k2 * d2 : d2,
                ] += kernels[:, :, i, j, k]

    # Remove input padding
    if ceil_mode:
        result = result[:, :, : result.size(2) - c0, : result.size(3) - c1, : result.size(4) - c2]
    result = result[:, :, p0 : result.size(2) - p0, p1 : result.size(3) - p1, p2 : result.size(4) - p2]
...
def _pool3d_reshape(
    ...
    """Rearrange a 5-d tensor so that each kernel is represented by each row"""
    ...
    assert isinstance(k, tuple), "kernel_size must be an int or tuple"
    assert isinstance(s, tuple), "stride must be and int, a tuple, or None"
    assert len(k) == 3, "kernel_size must be an int or tuple pair"
    assert len(s) == 3, "stride must be an int or tuple pair"
    assert isinstance(pad_value, int), "pad_value must be an integer"
    assert input.dim() >= 3, "Pooling input dimension should be at least 3"

    # Apply padding if necessary
    # if padding is not None:
    #     padding = (padding, padding) if isinstance(padding, int) else padding
    #     assert len(padding) == 3, "Padding must be an integer or a triple"
    ...
    n = input.size()[:-3]
    h = _pooling_output_shape(
        input.size(-3), k[0], padding[0], padding[1], s[0], d[0], ceil_mode
    )
    w = _pooling_output_shape(
        input.size(-2), k[1], padding[2], padding[3], s[1], d[1], ceil_mode
    )
    
    de = _pooling_output_shape(
        input.size(-1), k[2], padding[4], padding[5], s[2], d[2], ceil_mode
    ) 
    out_size = tuple(n + (h, w, de))

    input = torch.nn.functional.pad(input, padding, value=pad_value)
    if ceil_mode:
        update_pad = [0, 0, 0, 0, 0, 0]
        update_pad[3] = h * s[0] + (k[0] - 1) * d[0] - input.size(-3)
        update_pad[1] = w * s[1] + (k[1] - 1) * d[1] - input.size(-2)
        update_pad[5] = d * s[2] + (k[2] - 1) * d[2] - input.size(-1)
        input = torch.nn.functional.pad(input, tuple(update_pad), value=pad_value)

    # Reshape input to arrange kernels to be represented by rows
    kernel_indices = torch.tensor(range(0, k[1] * d[1], d[1]), device=input.device)
    kernel_indices = torch.cat(
        [kernel_indices + i * input.size(-2) for i in range(0, k[0] * d[0], d[0])]
    )
    kernel_indices = torch.cat(
        [kernel_indices + i * input.size(-2) * input.size(-3) for i in range(0, k[2]* d[2], d[2])] 
    )
    kernel_indices = torch.stack([kernel_indices + i * s[1] for i in range(w)])
    ...
    offset = input.size(-2) * input.size(-3)
    kernel_indices = torch.cat([kernel_indices + i * s[2] * offset for i in range(de)])
    offset = offset * input.size(-1)
    for dim in range(3, input.dim()):
        
        kernel_indices = torch.stack(
            [kernel_indices + i * offset for i in range(input.size(-dim - 1))]
        )
        offset *= input.size(-dim-1)
    output = input.take(kernel_indices)
    return output, out_size