...
from .common.util import _grad_input_padding, _grad_input_padding_3
...
@register_function("max_pool3d")
class AutogradMaxPool3D(AutogradFunction):
    @staticmethod
    def forward(
        ctx,
        input,
    ...
    def backward(ctx, grad_output):
        ...
        return grad_output._max_pool3d_backward(
            indices,
            kernel_size,
            padding=padding,
            stride=stride,
...
@register_function("conv3d")
class AutogradConv3D(AutogradFunction):
    @staticmethod
    def forward(ctx, input, kernel, stride=1, padding=0, dilation=1, groups=1):
        if isinstance(stride, (int, float)):
    ...
    def backward(ctx, grad_output):
        ...
        batch_size = input.size(0)
        # print(input.size())
        # print(input.size(dim = 1))
        if len(input.size()) == 5:
            out_channels, in_channels, kernel_size_z, kernel_size_y, kernel_size_x = kernel.size()
        else:
            out_channels, in_channels, kernel_size_y, kernel_size_x = kernel.size()
        in_channels *= groups
        assert input.size(1) == in_channels, "wrong number of input channels"
        ...
        # TODO: Implement conv3d gradient under following condition:
        if groups > 1 and input.size(1) > groups:
            raise NotImplementedError(
                "conv3d backward with groups > 1 and in_channels > groups not implemented"
            )

        # compute gradient with respect to input:
        output_padding = _grad_input_padding_3(
            grad_output,
            input.size(),
            stride,
            padding,
            (kernel_size_z, kernel_size_y, kernel_size_x),
            dilation=dilation,
        )
        grad_input = grad_output.conv_transpose3d(
            kernel,
            stride=stride,
            padding=padding,
            output_padding=output_padding,
        ...
        input = input.view(
            1, input.size(0) * input.size(1), input.size(2), input.size(3), input.size(4),
        )
        # dilation and stride are swapped based on PyTorch's conv3d_weight implementation
        grad_kernel = input.conv3d(
            grad_output,
            stride=dilation,
            padding=padding,
            dilation=stride,
        ...
        grad_kernel = grad_kernel.narrow(2, 0, kernel_size_z)
        grad_kernel = grad_kernel.narrow(3, 0, kernel_size_y)
        grad_kernel = grad_kernel.narrow(4, 0, kernel_size_x)
        return (grad_input, grad_kernel)
