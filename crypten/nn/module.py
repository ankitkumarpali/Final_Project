...
class Conv(Module):
    ...
    def forward(self, x):
        ...
        # unpack inputs:
        # print("len", len(x), len(x[0]), len(x[1]), len(x[2]))
        if len(x) == 2:
            x, weight = x
            bias = None
        elif len(x) == 3:
...
class Conv3d(Module):
    def __init__(
        ...
        # initialize model parameters:
        pytorch_module = torch.nn.Conv3d(
            in_channels,
            out_channels,
            kernel_size,
            stride=stride,
    ...
    def forward(self, x):
        ...
        if hasattr(self, "bias") and self.bias is not None:
            x = x.add(self.bias.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1))
        return x
...
class MaxPool3d(_Pool2d):
    """
    Module that performs 3D max pooling (see :meth:`AvgPool2d`)
    """
    ...
    @staticmethod
    def from_onnx(attributes=None):
        return super(MaxPool3d, MaxPool3d).from_onnx("max", attributes=attributes)
