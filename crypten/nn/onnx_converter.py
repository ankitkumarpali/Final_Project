...
def from_onnx(onnx_string_or_file):
    ...
    onnx.save(onnx_model, "model_graph1.onnx")
...
def from_pytorch(pytorch_model, dummy_input):
    ...
    f.close()
    # new_pytorch_model = STAN()
    # set model architecture to export model back to pytorch model
    # new_pytorch_model.load_state_dict(pytorch_model.state_dict())

    # set the new model as the PyTorch model of the crypten model
    # crypten_model.pytorch_model = new_pytorch_model
    # crypten_model.pytorch_model = copy.deepcopy(pytorch_model)
    crypten_model.pytorch_model = pytorch_model
